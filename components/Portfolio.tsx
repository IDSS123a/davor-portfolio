...
const SearchableDropdown: React.FC<SearchableDropdownProps> = ({ label, options, selected, onSelect }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="w-full bg-theme-light dark:bg-theme-dark border border-theme-accent/20 rounded-xl px-4 py-3 text-sm text-theme-dark dark:text-theme-light focus:outline-none focus:ring-2 focus:ring-theme-accent/50 text-left flex justify-between items-center shadow-sm"
      >
      ...
      {isOpen && (
        <div className="absolute z-[60] mt-2 w-full bg-theme-light dark:bg-theme-dark border border-theme-accent/20 rounded-xl shadow-2xl overflow-hidden animate-fade-in-up">
          <div className="p-2 border-b border-theme-accent/10">
            <input
              type="text"
              placeholder={`Search ${label}...`}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-theme-light/50 dark:bg-theme-dark/50 border border-theme-accent/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-theme-accent"
              autoFocus
            />
          </div>
          <div className="max-h-60 overflow-y-auto custom-scrollbar">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                    selected === opt ? 'bg-theme-accent text-white' : 'hover:bg-theme-accent/5 text-theme-dark dark:text-theme-light'
                  }`}
                >
                  {opt}
                </button>
              ...
              <div className="px-4 py-2 text-xs text-theme-dark/50 dark:text-theme-light/50 italic">No matches found</div>
            )}
          </div>
        </div>
      )}
    ...
    <section id="portfolio" className="py-24 bg-theme-light dark:bg-theme-dark relative">
          ...
          <div className="bg-theme-light/50 dark:bg-theme-dark/50 p-6 rounded-2xl border border-theme-accent/10 backdrop-blur-sm">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
              ...
              <div className="md:col-span-3">
                 <SearchableDropdown label="All Industries" options={industries} selected={selectedIndustry} onSelect={setSelectedIndustry} />
              </div>

              <div className="md:col-span-3">
                 <SearchableDropdown label="All Technologies" options={technologies} selected={selectedTech} onSelect={setSelectedTech} />
              </div>
           ...
           {visibleData.map((item) => (
             ...
             >
                <div className="relative h-48 overflow-hidden bg-theme-dark/10">
                   ...
                   <picture>
                      <source srcSet={`${item.imageUrl}?w=400 400w, ${item.imageUrl}?w=800 800w`} sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" />
                      <img 
                        src={item.imageUrl} 
                        alt={item.title} 
                        loading="lazy"
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
           <div className="absolute inset-0 bg-theme-dark/90 backdrop-blur-sm transition-opacity" onClick={() => setSelectedProject(null)}></div>
           <div className="relative w-full max-w-5xl bg-theme-light dark:bg-theme-dark rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-fade-in-up">
              <button onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 z-50 p-2 bg-theme-dark/20 hover:bg-theme-dark/40 dark:bg-theme-light/20 dark:hover:bg-theme-light/40 rounded-full text-theme-dark dark:text-theme-light transition-colors">
              ...
              <div className="w-full md:w-2/5 relative">
                 ...
                 <div className="absolute bottom-0 left-0 w-full p-8 text-white space-y-6">
                    <div><span className="bg-theme-accent text-white px-3 py-1 rounded text-xs font-bold uppercase tracking-widest">{selectedProject.industry}</span></div>
                    <div><p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1">Business Value</p><p className="text-lg font-medium leading-relaxed">{selectedProject.businessValue}</p></div>
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                       <div><p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1">ROI</p><p className="text-theme-accent font-bold text-xl">{selectedProject.roi}</p></div>
                       <div><p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1">Timeline</p><p className="font-bold text-xl">{selectedProject.implementationTime}</p></div>
                    </div>
                 </div>
              </div>
              <div className="w-full md:w-3/5 p-8 md:p-12 overflow-y-auto custom-scrollbar">
                 ...
                 <div className="space-y-8">
                    <div><h3 className="flex items-center gap-2 text-theme-accent font-bold uppercase tracking-widest text-xs mb-3"><span className="w-8 h-px bg-theme-accent"></span> The Challenge</h3><p className="text-theme-dark/80 dark:text-theme-light/80 leading-relaxed text-lg">{selectedProject.problem}</p></div>
                    <div><h3 className="flex items-center gap-2 text-theme-accent font-bold uppercase tracking-widest text-xs mb-3"><span className="w-8 h-px bg-theme-accent"></span> The AI Solution</h3><p className="text-theme-dark/80 dark:text-theme-light/80 leading-relaxed text-lg">{selectedProject.fullDescription}</p></div>
                    <div><h3 className="flex items-center gap-2 text-theme-accent font-bold uppercase tracking-widest text-xs mb-3"><span className="w-8 h-px bg-theme-accent"></span> Technologies</h3><div className="flex flex-wrap gap-2">{selectedProject.technologies.map(tech => (<span key={tech} className="bg-theme-light dark:bg-theme-dark border border-theme-accent/20 text-theme-dark/70 dark:text-theme-light/70 px-3 py-1.5 rounded-lg text-sm font-medium">{tech}</span>))}</div></div>
                 </div>
                 <div className="mt-12 pt-8 border-t border-theme-accent/10 flex justify-end">
                    <button onClick={() => setSelectedProject(null)} className="px-6 py-2 bg-theme-dark dark:bg-theme-light text-white dark:text-theme-dark font-bold rounded-lg hover:opacity-90 transition-opacity">Close Project</button>
                 </div>
              </div>
           </div>
        </div>
